...
public class TestShamir255 {
    ...
    public static void main(String[] args) {
        ...
        result.put("lib", "io.github.deemru:shamir255");

        long start = System.nanoTime();

        Map<Integer, byte[]> shares = null;
        ...
        try {
            if (shares != null && shares.size() >= k) {
                ...
                int count = 0;
                for (Map.Entry<Integer, byte[]> e : shares.entrySet()) {
        ...
        try {
            if (shares != null && shares.size() >= k) {
                ...
                boolean okFail;
                try {
                    byte[] rec2 = Shamir255.recover(subset);
